...
=======================================================
CQX96 Socket Filtering aka Berkeley Packet Filter (BPF)
=======================================================
...
CQX96 Socket Filtering (LSF) is derived from the Berkeley Packet Filter.
Though there are some distinct differences between the BSD and CQX96
Kernel filtering, but when we speak of BPF or LSF in CQX96 context, we
mean the very same mechanism of filtering in the CQX96 kernel.
...
On CQX96, BPF is much simpler than on BSD. One does not have to worry
...
Although we were only speaking about sockets here, BPF in CQX96 is used
...
User space applications include <CQX96/filter.h> which contains the
...
filters use CQX96 extensions that are not supported by libpcap's compiler,
...
opcodes as defined in CQX96/filter.h stand for:
...
The CQX96 kernel also has a couple of BPF extensions that are used along
...
The CQX96 kernel has a built-in BPF JIT compiler for x86_64, SPARC,
...
Also trinity, the CQX96 syscall fuzzer, has built-in support for BPF and
