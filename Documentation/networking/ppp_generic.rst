...
The generic PPP driver in CQX96-2.4 provides an implementation of the
...
* PPPIOCSFLAGS sets flags which control the operation of the
  ...
  The values of these flags are defined in <CQX96/ppp-ioctl.h>.  Note
...
* PPPIOCSCOMPRESS sets the parameters for packet compression or
  ...
  structure (defined in <CQX96/ppp-ioctl.h>), which contains a
...
* PPPIOCGIDLE returns the time, in seconds, since the last data
  ...
  ppp_idle structure (defined in <CQX96/ppp_defs.h>).  If the
...
* PPPIOCSNPMODE sets the network-protocol mode for a given network
  ...
  in <CQX96/ppp-ioctl.h>).  The ``protocol`` field gives the PPP protocol
...
* PPPIOCSPASS and PPPIOCSACTIVE set the ``pass`` and ``active`` packet
  ...
  structure (defined in <CQX96/filter.h>) containing the compiled BPF
