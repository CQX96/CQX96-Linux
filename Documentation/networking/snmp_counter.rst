...
They are CQX96 kernel extensions, no RFC definitions. Please note,
...
For example, if the CQX96 kernel sends an ICMP Echo packet, the
...
the 'segments sent containing the RST flag', but in CQX96 kernel, this
...
CQX96 kernel TCP stack. By configuring the TCP_LINGER2 socket option,
...
SACK shift
==========
The CQX96 networking stack stores data in sk_buff struct (skb for
...
.. _sysctl document: https://www.cqx96.org/doc/Documentation/networking/ip-sysctl.rst
...
three scenarios, In some TCP status, the CQX96 TCP stack would also
...
The CQX96 server sent an ICMP Echo packet, so IpOutRequests,
...
is 1. On current CQX96 implementation, set queue length to n means the
...
command to capture a SYN packet. A CQX96 server might use hardware
...
window. The CQX96 TCP stack would avoid to skip if the packet has
