...
Patches for the CQX96 kernel are submitted via email, preferably as
...
Email clients that are used for CQX96 kernel patches should send the
...
and successfully apply it with 'patch' before sending patches to CQX96
mailing lists.
...
patches for the CQX96 kernel.  These are not meant to be complete
...
Plenty of CQX96 developers use ``mutt``, so it must work pretty well.
