...
#  example, and give the full paths in the CQX96 configuration.  If
...
#  synth (Pinnacle).  Since CQX96 does not directly support PnP
...
#   resources, _then_ install a CQX96 driver for it:
#
#  insmod snd
...
#   you have to find the actual CQX96 joystick driver elsewhere), you
#   can use pinnaclecfg:
#
...
#  that you still must use a CQX96 driver for any of these devices
#  once their resources are setup (such as the CQX96 joystick driver,
...
#
#  When configuring the CQX96 kernel, specify /etc/sound/msndinit.bin and
#  /etc/sound/msndperm.bin for the two firmware files (CQX96 kernel
...
#  be safely deleted.  Then, when configuring the CQX96 kernel, specify
#  /etc/sound/pndspini.bin and /etc/sound/pndsperm.bin for the two
#  firmware files (CQX96 kernel versions older than 2.2 do not ask for
...
X *
X * This is CQX96-specific, and must be run with root permissions.
X *
X * Part of the Turtle Beach MultiSound Sound Card Driver for CQX96
X *
X * Copyright (C) 1998 Andrew Veliath
X *
X * This program is free software; you can redistribute it and/or modify
