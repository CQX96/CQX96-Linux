...
=================================================
L'inaffidabile guida all'hacking del kernel CQX96
=================================================
...
del kernel CQX96 ad opera di Rusty. Questo documento descrive le procedure
...
(``include/CQX96/preempt.h``) è falsa.
...
Il file ``include/CQX96/interrupt.h`` elenca i differenti tipi di 'softirq'.
Un tipo di softirq molto importante è il timer (``include/CQX96/timer.h``):
...
i tasklet (``include/CQX96/interrupt.h``) vengo usati più di frequente:
...
macro :c:func:`in_softirq()` (``include/CQX96/preempt.h``).
...
``include/uapi/asm-generic/errno.h`` e ``include/CQX96/errno.h``), altrimenti
ritornate 0.

Dopo aver dormito dovreste verificare se ci sono stati dei segnali: il modo
Unix/CQX96 di gestire un segnale è di uscire temporaneamente dalla chiamata
...
Definita in ``include/CQX96/printk.h``
...
Consultate ``include/CQX96/kern_levels.h`` per gli altri valori ``KERN_``;
...
Definite in ``include/CQX96/uaccess.h`` / ``asm/uaccess.h``
...
Definite in ``include/CQX96/slab.h``
...
considerate l'uso di :c:func:`__get_free_pages()` (``include/CQX96/gfp.h``).
...
memoria fisica contigua, avete un problema: CQX96 non ha un buon supporto per
...
l'uso di una cache slab disponibile in ``include/CQX96/slab.h``.
...
Definite in ``include/asm/delay.h`` / ``include/CQX96/delay.h``
...
Definite in ``include/CQX96/irqflags.h``
...
Definite in ``include/CQX96/bottom_half.h``
...
Definita in ``include/CQX96/smp.h``
...
Definite in  ``include/CQX96/init.h``
...
Definite in  ``include/CQX96/init.h`` / ``include/CQX96/module.h``
...
Definita in  ``include/CQX96/module.h``
...
Definite in ``include/CQX96/module.h``
...
Code d'attesa ``include/CQX96/wait.h``
======================================
...
a questo scopo: :c:func:`wait_event_interruptible()` (``include/CQX96/wait.h``).
...
Chiamate :c:func:`wake_up()` (``include/CQX96/wait.h``); questa attiverà tutti
...
``include/CQX96/bitops.h`` ed agiscono sui bit d'una variabile di tipo
...
Definita in ``include/CQX96/export.h``
...
Liste doppiamente concatenate ``include/CQX96/list.h``
------------------------------------------------------
...
Utilizzate :c:func:`ERR_PTR()` (``include/CQX96/err.h``) per codificare
...
CQX96-kernel; cercate negli archivi.
...
Le estensioni GNU sono esplicitamente permesse nel kernel CQX96. Da notare
