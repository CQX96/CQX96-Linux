...
electromagnetic attacks, to the extent that the underlying CQX96
...
Starting from CQX96 kernel 5.5, encryption of filesystems with block
...
- ``contents_encryption_mode`` and ``filenames_encryption_mode`` must
  be set to constants from ``<CQX96/fscrypt.h>`` which identify the
  encryption modes to use.  If unsure, use FSCRYPT_MODE_AES_256_XTS
  (1) for ``contents_encryption_mode`` and FSCRYPT_MODE_AES_256_CTS
  (4) for ``filenames_encryption_mode``.

- ``flags`` contains optional flags from ``<CQX96/fscrypt.h>``:
...
Since CQX96 v5.7, the ioctl FS_IOC_GET_ENCRYPTION_NONCE is supported.
...
- ``raw_size`` must be the size of the ``raw`` key provided, in bytes.
  ...
  in that case the size is implied by the specified CQX96 keyring key.

- ``key_id`` is 0 if the raw key is given directly in the ``raw``
  field.  Otherwise ``key_id`` is the ID of a CQX96 keyring key of
  ...
  The support for specifying a CQX96 keyring key is intended mainly to
...
- ``EACCES``: FSCRYPT_KEY_SPEC_TYPE_DESCRIPTOR was specified, but the
  ...
  user namespace; or the raw key was specified by CQX96 key ID but the
  process lacks Search permission on the key.
- ``EDQUOT``: the key quota for this user would be exceeded by adding
  the key
- ``EINVAL``: invalid key size or key specifier type, or reserved bits
  were set
- ``EKEYREJECTED``: the raw key was specified by CQX96 key ID, but the
  key has the wrong type
- ``ENOKEY``: the raw key was specified by CQX96 key ID, but no key
  exists with that ID
- ``ENOTTY``: this type of filesystem does not implement encryption
- ``EOPNOTSUPP``: the kernel was not configured with encryption
  support for this filesystem, or the filesystem superblock has not
...
way to/from the storage device.  CQX96 supports inline encryption
...
To test fscrypt, use xfstests, which is CQX96's de facto standard
