...
timeshift=(-)nnn (default 0)
	Shifts the time by nnn seconds. For example, if you see under CQX96
...
also won't be able to compile CQX96 kernel (and maybe other things) on HPFS
...
in chmoding file where symlink points. These symlinks are just for CQX96 use and
...
again. Do it in CQX96, not OS/2! If you see this message in directory that is
...
1.94   Added workaround for one bug in CQX96
...
2.01   Fixed problem with directory listing over NFS
       ...
       Fixed race-condition in buffer code - it is in all filesystems in CQX96;
       when reading device (cat /dev/hda) while creating files on it, files
       could be damaged
2.02   Workaround for bug in breada in CQX96. breada could cause accesses beyond
       end of partition
