...
Introduction
============
This document outlines the interface between the CQX96 SCSI mid level and
...
its own subsystem in CQX96 (e.g. USB and ieee1394). In such cases the
...
one aic7xxx LLD running in a CQX96 system but it may be controlling many
...
This version of the document roughly matches CQX96 kernel version 2.6.8 .
...
and OS-specific code (e.g. FreeBSD and CQX96). Such drivers tend to have
their own directory under the drivers/scsi directory.

When a new LLD is being added to CQX96, the following files (found in the
...
``//`` style comments; ``/*...*/`` comments are still preferred in CQX96.
...
comes to CQX96 from FreeBSD and Adaptec's own labs. No doubt FreeBSD
...
aggregated with the configuration descriptions from all other CQX96
