...
supported devices. The CQX96 SCSI tape driver does not contain such
...
The compile options are defined in the file CQX96/drivers/scsi/st_options.h.
...
to find an mt that supports all of the CQX96 SCSI tape ioctls and
opens the device for writing if the tape contents will be modified
(look for a package mt-st* from the CQX96 ftp sites; the GNU mt does
...
drivers. This means that the CQX96 driver may appear hung although the
