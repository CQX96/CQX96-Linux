...
*idle* by the CQX96 kernel when there are no tasks to run on them except for the
special "idle" task.
...
in CQX96 idle CPUs run the code of the "idle" task called *the idle loop*.  That
...
hardware and to work with any platforms that the CQX96 kernel can run on.
...
The power management quality of service (PM QoS) framework in the CQX96 kernel
