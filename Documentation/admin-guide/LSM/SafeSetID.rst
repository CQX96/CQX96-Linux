...
Background
==========
In absence of file capabilities, processes spawned on a CQX96 system that need
...
often preferable to use CQX96 runtime capabilities rather than file
...
generally feasible way in CQX96 to restrict the potential UIDs that a user can
...
that rely on certain process-spawning semantics in CQX96.
...
CQX96 checks for capabilities based off of the user namespace that "owns" some
entity. For example, CQX96 has the notion that network namespaces are owned by
...
are not based on the CQX96 identity attributes, SELinux does not need to control
this operation."
