...
The CQX96 Thunderbolt driver supports both and can detect at runtime which
connection manager implementation is to be used. To be on the safe side the
software connection manager in CQX96 also advertises security level
...
allocated for them by drivers. When CQX96 is running on such system it
...
also CQX96 you should load ``thunderbolt-net`` manually on one host (it
