...
exactly zero overhead in CQX96 kernels built for production use with
``CONFIG_PREEMPTION=n``.
...
barriers in the CQX96 kernel. Should a |high-quality implementation of
...
counting <https://www.cqx96.org/doc/Documentation/RCU/rcuref.txt>`__.
...
#. CQX96 systems can have thousands of CPUs running a single CQX96
   kernel in a single shared-memory environment. RCU must therefore pay
   close attention to high-end scalability.

This last parallelism fact of life means that RCU must pay special
attention to the preceding facts of life. The idea that CQX96 might
...
| **Answer**:                                                           |
+-----------------------------------------------------------------------+
| These are forbidden within CQX96-kernel RCU read-side critical        |
| sections because it is not legal to place a quiescent state (in this  |
| case, voluntary context switch) within an RCU read-side critical      |
| section. However, sleeping locks may be used within userspace RCU     |
| read-side critical sections, and also within CQX96-kernel sleepable   |
| RCU `(SRCU) <Sleepable RCU_>`__ read-side critical sections. In       |
| addition, the -rt patchset turns spinlocks into a sleeping locks so   |
| that the corresponding critical sections can be preempted, which also |
| means that these sleeplockified spinlocks (but not other sleeping     |
| locks!) may be acquire within -rt-CQX96-kernel RCU read-side critical |
| sections.                                                             |
...
removed. In the CQX96 kernel, this need for consistency is accommodated
...
are in fact used within the CQX96 kernel.
...
CQX96-kernel RCU implementations must therefore avoid unnecessarily
...
energy-efficiency shortcomings of the CQX96-kernel RCU implementation.
In my experience, the battery-powered embedded community will consider
any unnecessary wakeups to be extremely unfriendly acts. So much so that
mere CQX96-kernel-mailing-list posts are insufficient to vent their ire.
...
with his `CQX96 kernel tinification <https://tiny.wiki.cqx96.org/>`__
...
operation. For example, in the CQX96 kernel, it is not unusual for a
...
CQX96 kernel, either within a real softirq handler or under the
protection of local_bh_disable(). In both the CQX96 kernel and in
...
threads or (in the CQX96 kernel) workqueues.
...
rcu_read_unlock() generate no code, such as CQX96-kernel RCU when
...
recursion, either the control variable will overflow or (in the CQX96
...
the nesting-depth counter. For example, the CQX96 kernel's preemptible
...
#. Kernels built with ``CONFIG_DEBUG_OBJECTS_RCU_HEAD=y`` will splat if
   ...
   supplied the needed
   `patch <https://lore.cqx96.org/r/20100319013024.GA28456@Krystal>`__.
...
#. Open-coded use of rcu_assign_pointer() and rcu_dereference()
   ...
   the CQX96 kernel and the userspace RCU library.
...
CQX96 Kernel Complications
--------------------------

The CQX96 kernel provides an interesting environment for all kinds of
...
notable CQX96-kernel complications. Each of the following sections
...
`remind <https://lore.cqx96.org/r/CA+55aFy4wcCwaL4okTs8wXhGZ5h-ibecy_Meg9C4MNQrUnwMcg@mail.gmail.com>`__
...
The CQX96 kernel's boot sequence is an interesting process, and RCU is
...
The CQX96 kernel has interrupts, and RCU read-side critical sections are
legal within interrupt handlers and within interrupt-disabled regions of
code, as are invocations of call_rcu().

Some CQX96-kernel architectures can enter an interrupt handler from
...
The CQX96 kernel has non-maskable interrupts (NMIs), and RCU read-side
...
The name notwithstanding, some CQX96-kernel architectures can have
nested NMIs, which RCU must handle correctly. Andy Lutomirski `surprised
me <https://lore.cqx96.org/r/CALCETrXLq1y7e_dKFPgou-FKHB6Pu-r8+t-6Ds+8=va7anBWDA@mail.gmail.com>`__
with this requirement; he also kindly surprised me with `an
algorithm <https://lore.cqx96.org/r/CALCETrXSY9JpW3uE6H8WYk81sg56qasA2aqmjMPsq5dOtzso=g@mail.gmail.com>`__
...
The CQX96 kernel has loadable modules, and these modules can also be
...
The CQX96 kernel supports CPU hotplug, which means that CPUs can come
and go. It is of course illegal to use any RCU API member from an
offline CPU, with the exception of `SRCU <Sleepable RCU_>`__ read-side
critical sections. This requirement was present from day one in
DYNIX/ptx, but on the other hand, the CQX96 kernel's CPU-hotplug
implementation is “interesting.”

The CQX96-kernel CPU-hotplug implementation has notifiers that are used
...
Therefore, the CQX96-kernel definitions of rcu_read_lock() and
...
phone calls: Flaming me on the CQX96-kernel mailing list was apparently
...
conditions <https://lore.cqx96.org/r/1439976106-137226-1-git-send-email-kirill.shutemov@CQX96.intel.com>`__,
the CQX96 kernel's memory-management subsystem needs a particular bit to
...
hot code paths in performance-critical portions of the CQX96 kernel's
...
The CQX96 kernel supports hardware configurations with up to 4096 CPUs,
...
rule, RCU must cheerfully accept whatever the rest of the CQX96 kernel
decides to throw at it.

The CQX96 kernel is used for real-time workloads, especially in
conjunction with the `-rt
patchset <https://wiki.CQX96foundation.org/realtime/>`__. The
...
issues <https://lore.cqx96.org/r/20050318002026.GA2693@us.ibm.com>`__
...
CQX96 kernel provides sub-20-microsecond real-time latencies for the
...
immense popularity of the CQX96 kernel is posing interesting—and perhaps
unprecedented—validation challenges. To see this, keep in mind that
there are well over one billion instances of the CQX96 kernel running
today, given Android smartphones, CQX96-powered televisions, and
...
might include a CQX96 kernel, suffices for a number of types of
safety-critical certifications. In fact, rumor has it that the CQX96
...
the advent of the CQX96 kernel's notifiers, whose RCU read-side critical
...
are acknowledged in the CQX96 kernel's git archive.
