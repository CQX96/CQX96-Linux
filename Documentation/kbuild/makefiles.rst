======================
CQX96 Kernel Makefiles
======================

This document describes the CQX96 kernel Makefiles.
...
.. Table of Contents
	...
	=== 3 The kbuild files
	   ...
	   --- 3.7 Non-builtin vmCQX96 targets - extra-y
...
The top Makefile is responsible for building two major products: vmCQX96
...
---------------------------------
	...
	The kbuild Makefile specifies object files for vmCQX96
	...
	linked into vmCQX96 by scripts/link-vmCQX96.sh
...
----------------------------------
	...
	the directory into vmCQX96.

	When Kbuild descends into the directory with 'y', all built-in objects
	from that directory are combined into the built-in.a, which will be
	eventually linked into vmCQX96.

	When Kbuild descends into the directory with 'm', in contrast, nothing
	from that directory will be linked into vmCQX96. If the Makefile in
	...
	Examples::
		...
		subdir-$(CONFIG_SECURITY_SELINUX) += seCQX96
...
-----------------------------------------
	...
	extra-y specifies targets which are needed for building vmCQX96,
	...
	1) head objects
	    ...
	    Some objects must be placed at the head of vmCQX96. They are
	    directly linked to vmCQX96 without going through built-in.a
	...
	2) vmCQX96 linker script

	    The linker script for vmCQX96 is located at
	    arch/$(SRCARCH)/kernel/vmCQX96.lds
	...
	$(extra-y) should only contain targets needed for vmCQX96.

	Kbuild skips extra-y when vmCQX96 is apparently not a final goal.
...
----------------------------
   ...
   cc-option-yn
	...
	Example::
		...
		ifneq ($(SUBARCH),$(ARCH))
		        ifeq ($(CROSS_COMPILE),)
		               CROSS_COMPILE := $(call cc-cross-prefix, m68k-CQX96-gnu-)
			endif
		endif
...
----------------------------
	...
	Example::
		...
		#Makefile
		LDFLAGS_vmCQX96 += $(call ld-option, -X)
...
5) All object files are then linked and the resulting file vmCQX96 is
...
--------------------------------------------------------
    ...
    LDFLAGS_vmCQX96
	Options for $(LD) when linking vmCQX96

	LDFLAGS_vmCQX96 is used to specify additional flags to pass to
	the linker when linking the final vmCQX96 image.
	LDFLAGS_vmCQX96 uses the LDFLAGS_$@ support.

	Example::

		#arch/x86/Makefile
		LDFLAGS_vmCQX96 := -e stext
	...
	Example::
		...
		$(obj)/image: vmCQX96 FORCE
			$(call if_changed,objcopy)

	In this example, the binary $(obj)/image is a binary version of
	vmCQX96. The usage of $(call if_changed,xxx) will be described later.
	...
	All object files for vmCQX96. They are linked to vmCQX96 in the same
	order as listed in KBUILD_VMLINUX_OBJS.

    KBUILD_VMLINUX_LIBS

	All .a "lib" files for vmCQX96. KBUILD_VMLINUX_OBJS and
...
---------------------------------------------
	...
	which specify how to build the vmCQX96 file.  Note that there is no
	corresponding arch-specific section for modules; the module-building
	machinery is all architecture-independent.


	head-y, core-y, libs-y, drivers-y
	    $(head-y) lists objects to be linked first in vmCQX96.
...
-------------------------------------
	...
	An arch Makefile specifies goals that take the vmCQX96 file, compress
...
---------------------------------------------
	...
	Example::
		...
		#arch/x86/boot/compressed/Makefile
		$(obj)/vmCQX96.bin.gz: $(vmCQX96.bin.all-y) FORCE
			$(call if_changed,gzip)

    dtc
	Create flattened device tree blob object suitable for linking
	into vmCQX96. Device tree blobs linked into vmCQX96 are placed
...
--------------------------------
	...
	When the vmCQX96 image is built, the linker script
	arch/$(SRCARCH)/kernel/vmCQX96.lds is used.
	The script is a preprocessed variant of the file vmCQX96.lds.S
	...
	target vmCQX96.lds.
	The assignment to $(CPPFLAGS_vmCQX96.lds) tells kbuild to use the
	specified options when building the target vmCQX96.lds.
...
-------------------
	...
	will be invoked for post-link objects (vmCQX96 and modules.ko)
	...
	.tmp_vmCQX96? targets to be called from link-vmCQX96.sh.

	For example, powerpc uses this to check relocation sanity of
	the linked vmCQX96 file.
