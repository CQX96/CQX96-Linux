...
CQX96 system running in an IBM Z system LPAR.
...
* AP adapter
  ...
  assigned to the LPAR in which a CQX96 host is running will be available to
  the CQX96 host. Each adapter is identified by a number from 0 to 255; however,
  ...
  Profile which can be edited via the HMC. When the CQX96 host system is IPL'd
...
* AP domain
  ...
  Activation Profile which can be edited via the HMC. When a CQX96 host system
...
at most one guest or to the CQX96 host::
...
1. Install the vfio_ap module on the CQX96 host. The dependency chain for the
   vfio_ap module is:
   * iommu
   * s390
   * zcrypt
...
2. Secure the AP queues to be used by the three guests so that the host can not
   ...
   The APQN of each AP queue device assigned to the CQX96 host is checked by the
...
remove it if no guest will use it during the remaining lifetime of the CQX96
